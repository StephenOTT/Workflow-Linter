...
/**
 * Linter Rule Configuration that transforms linting rules in YAML into Element Validators
 */
        ...
        linterRules.forEach { lr ->
            ...
            lr.elementTypes!!.forEach { elementType ->
                ...
                lr.headerRule?.let { headerRule ->
                    CommonLinterRules.processRequiredHeadersRule(elementClass, headerRule.requiredKeys, lr.target)?.let { v ->
                        myList.add(v)
                    }

                    if (!headerRule.allowedNonDefinedKeys){
                        CommonLinterRules.processOptionalHeadersRule(elementClass, headerRule.optionalKeys, headerRule.requiredKeys, lr.target)?.let { v ->
                            myList.add(v)
                        }
                    }

                    if (!headerRule.allowedDuplicateKeys){
                        CommonLinterRules.processDuplicateHeaderKeysRule(elementClass, lr.target)?.let { v ->
                            myList.add(v)
                        }
                    }
                }
                ...
                lr.serviceTaskRule?.let { serviceTaskRule ->
                    ...
                    serviceTaskRule.allowedTypes?.let { types ->
                        CommonLinterRules.processServiceTaskAllowedTypesListRule(elementClass, types, lr.target)?.let { v ->
                            myList.add(v)
                        }
                    }
                }
